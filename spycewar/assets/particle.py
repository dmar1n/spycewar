...
class Particle(Sprite):
    ...
    def fade_rate(self) -> float:
        """Return the fade rate of the particle."""
        return self.__fade_rate

    def update(self, delta_time: int) -> None:
        """Update the particle's position based on the direction and speed."""
    ...
    def __check_alpha(self) -> None:
        """Check if the particle is transparent and removes it if it is."""
        if self.alpha <= 0:
            self.kill()

    def __check_position(self) -> None:
        """Check if the particle is off-screen and removes it if it is."""
