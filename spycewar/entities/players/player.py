...
class Player(GameObject):
    """Represent the player object in the game.
    ...
    def player_id(self) -> PlayerId:
        """Return tse player id of the player object."""
    ...
    def update(self, delta_time: float) -> None:
        """Update the player's position based on the input and the time passed since the last frame.
    ...
    def render(self, surface_dst: Surface) -> None:
        """Render the player to the given surface at the player's position.
    ...
    def __render_player_info(self, surface_dst: Surface) -> None:
        """Render the player's information to the given surface for debugging purposes.
        ...
        surface_dst.blit(
            render_text(font, f"Angle: {self.__ship_state.angle:.2f}"),
            (x, 50),
        )
        ...
        surface_dst.blit(
            render_text(font, f"Velocity: {self.__ship_state.velocity}"),
            (x, 110),
        )
        ...
        surface_dst.blit(
            render_text(font, f"hyperspace Cooldown: {self.hyperspace_cooldown:.2f}"),
            (x, 170),
        )
        surface_dst.blit(render_text(font, f"Shielded: {self.state.shield}"), (x, 190))

    def __wrap_position(self, surface_dst: Surface) -> None:
        """Wrap the player's position around the screen if it goes out of bounds.
    ...
    def __rotate_image(self) -> None:
        """Rotate the player image to the current angle if it has changed since the last frame."""
        if self.__last_angle != self.__ship_state.angle:
            self.__rotated_image = pygame.transform.rotate(
                self.image,
    ...
    def __thrust(self) -> None:
        """Apply full thrust to the player's velocity."""
        velocity, position = self.__compute_trajectory(
            backwards=True,
            offset=-5,
            speed=2.5,
        )
    ...
    def __shield(self, delta_time: float) -> None:
        """Apply a shield to the player's ship."""
    ...
    def __compute_trajectory(
        self,
        speed: float,
        backwards: bool = False,
        offset: int = 10,
    ) -> tuple[Vector2, Vector2]:
        """Compute the trajectory of the object based on the player's position and angle.

        Args:
            speed: the speed of the object to be spawned.
    ...
    def __update_velocity(self) -> None:
        """Update the player's velocity based on the current angle and acceleration.
