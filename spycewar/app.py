"""Module for initialising and running the main game application using Pygame.
...
from spycewar.constants import (
    GAME_NAME,
    SCREEN_HEIGHT_ENV_VAR,
    SCREEN_WIDTH_ENV_VAR,
)
...
class App:
    ...
    def __initialise_screen(self) -> pygame.Surface:
        """Initialise the game window with the screen size, resizable, and 32-bit color (with transparency)."""
    ...
    def __process_events(self) -> None:
        """Handle all events captured from the Pygame event queue.
    ...
    def __handle_resize_event(self, event: Event) -> None:
        if event.type == VIDEORESIZE:
            ...
            logger.debug("Resizing screen to: %s", screen_size)
    ...
    def __handle_background_color(self, event: Event) -> None:
        """Handle the gameover event to stop the game.
    ...
    def __handle_quit_event(self, event: Event) -> None:
        """Handle events to stop the game.
